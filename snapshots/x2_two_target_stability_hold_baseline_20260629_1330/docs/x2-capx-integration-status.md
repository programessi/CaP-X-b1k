...
BEHAVIOR tabletop workflow. It separates what is already in the stable path
from what still needs local non-oracle evidence.
...
The remaining acceptance gap is a fresh direct-API non-oracle stability run
from the user's terminal. The managed assistant environment cannot send the
CaP-X task/API prompt to an external LLM provider, so this one evidence item
must be produced outside the sandbox.
...
## Non-Oracle Evidence To Collect
...
After direct API evidence has been collected, strict mode should pass:
...
## What Remains After Direct API Passes

After the manual direct-API run passes, the X2 CaP-X tabletop integration can
be treated as complete for the current simulation scope. The next work would be
new scope:
