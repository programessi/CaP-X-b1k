...
The script-level index is:

```text
scripts/README.md
```

Use that file to distinguish stable smoke entry points from debug helpers and
...
### 2026-06-29 13:30 - Two-Target Oracle Stability And Hold-Step Hardening

Purpose:

- Run the two-target task repeatedly enough to catch non-deterministic visual
  grasp / execution failures.
- Keep oracle stability separate from codex-a stability because the codex-a
  stability loop sends task/workspace context to an external provider and was
  blocked by sandbox data-exfiltration policy.

Right oracle stability result before the hold-step change:
...
Left oracle stability initially failed in:
...
The visual chain was healthy in that failed run: OWL-ViT/SAM2 detected the
cube, Contact-GraspNet produced candidates, and the selected TCP target was
near the object. The failure was concentrated in final execution tracking:

```text
X2_TWO_TARGET_RESULT ok=False target=left
before_close_tcp_error_m=0.07700883218241314
before_close_ori_error_rad=0.16741965628642944
object_in_hand_after_close=False
place_error_m=None
...
The fix was deliberately scoped to action timing, not visual selection:

```text
execute_tcp_grasp_plan insertion max_steps: 240 -> 420
execute_tcp_grasp_plan fine-align max_steps: 360 -> 560
execute_tcp_grasp_plan fine-align settle_steps: max(20, settle_steps)
execute_tcp_grasp_plan fine-align hold_steps_per_waypoint: max(8, insert_hold_steps_per_waypoint)
pick_and_place_visual_object insert_hold_steps_per_waypoint: 5 -> 8
```

Left oracle stability after the hold-step change:
...
The working files were frozen after this fix in:

```text
snapshots/x2_two_target_stability_hold_baseline_20260629_1330
```

A follow-up right/left oracle stability rerun after the snapshot also passed.
This was run instead of codex-a stability because the codex-a loop sends
workspace/task prompt data to an external provider and was rejected by the
current data-exfiltration policy.
...
Rerun visual artifacts:

```text
outputs/x2_visual_artifacts/stability/two_targets_right_oracle_stability_20260629_1345_oracle_stability_after_hold_run01/x2_pick_place_red_cube_20260629_140824_446/
outputs/x2_visual_artifacts/stability/two_targets_left_oracle_stability_20260629_1345_oracle_stability_after_hold_run01/x2_pick_place_red_cube_20260629_141256_191/
```

### 2026-06-29 - Non-Oracle Stability Scripts Added

When the GPT API key is configured through the local `codex-a` command, use the
Codex CLI proxy scripts:
...
The generic direct-API path remains available for an explicitly configured
...
The generic direct API scripts call the configured OpenAI-compatible
`SERVER_URL` directly and do not start `codex_cli_server.py`.

After a manual non-oracle run, summarize outputs with:
...
Then run the local acceptance gate:
...
The integration audit reports whether the local baseline is present and whether
direct API evidence has closed the final gap:
...
The managed assistant environment cannot execute this external-LLM run because
it would send CaP-X task/API prompt text to an external provider. This is a
tooling policy limitation. It does not affect oracle validation or local user
terminal execution.
