...
class X2ControlApi(ApiBase):
    ...
    def _maybe_save_visual_grasp_artifacts(
        self,
        *,
        object_name: str,
        rgb: np.ndarray,
        ...
        grasp_plan: dict[str, Any] | None,
        selection: dict[str, Any] | None,
        grasp_tcp_pose: tuple[np.ndarray, np.ndarray] | None = None,
        precontact_tcp_pose: tuple[np.ndarray, np.ndarray] | None = None,
        tcp_axis_world: np.ndarray | None = None,
    ) -> str | None:
        artifact_root = os.environ.get("CAPX_X2_VISUAL_ARTIFACT_DIR")
        ...
        try:
            ...
            run_dir = root / f"{object_name}_{time.strftime('%Y%m%d_%H%M%S')}_{int(time.time() * 1000) % 1000:03d}"
            run_dir.mkdir(parents=True, exist_ok=True)

            rgb_u8 = self._rgb_u8(rgb)
            rgb_img = Image.fromarray(rgb_u8)
            rgb_img.save(run_dir / "rgb.png")

            mask_bool = np.asarray(mask, dtype=bool)
            mask_img = Image.fromarray((mask_bool.astype(np.uint8) * 255), mode="L")
            mask_img.save(run_dir / "sam2_mask.png")

            detection_overlay = rgb_img.copy()
            draw = ImageDraw.Draw(detection_overlay)
            box = detection.get("box")
            if box is not None:
                x1, y1, x2, y2 = [float(v) for v in box]
                draw.rectangle((x1, y1, x2, y2), outline=(255, 255, 0), width=3)
            detection_overlay.save(run_dir / "detection_overlay.png")

            overlay = rgb_u8.copy()
            red = np.array([255, 0, 0], dtype=np.uint8)
            overlay[mask_bool] = (0.55 * overlay[mask_bool] + 0.45 * red).astype(np.uint8)
            Image.fromarray(overlay).save(run_dir / "sam2_mask_overlay.png")
                    ...
                    ranked.append(
                        {
                            ...
                            "adapted_position": self._jsonable(item.get("adapted_position")),
                            "adapted_quat_xyzw": self._jsonable(item.get("adapted_quat_xyzw")),
            ...
            summary = {
                ...
                "detection": self._jsonable(detection),
                "detections": self._jsonable(detections),
                "mask_score": self._jsonable(mask_result.get("score")),
                "mask_pixels": int(mask_bool.sum()),
                "pose_estimate": {
                    "meaning": "T_world_object estimated from SAM2 mask and RGB-D depth",
                    "position_world": self._jsonable(object_pos),
                    "quat_xyzw_world": self._jsonable(object_quat),
                    "bbox_extent": self._jsonable(bbox_extent),
                },
                "graspnet_candidate_count": None if grasp_plan is None else grasp_plan.get("candidate_count"),
                "selection": None
                if selection is None
                    ...
                    "tcp_axis_world": self._jsonable(selection.get("tcp_axis_world")),
                    "ranked_candidates": ranked,
                },
                "grasp_tcp_pose": self._jsonable(grasp_tcp_pose),
                "precontact_tcp_pose": self._jsonable(precontact_tcp_pose),
                "tcp_axis_world": self._jsonable(tcp_axis_world),
                "contract": "grasp_tcp_pose and precontact_tcp_pose are T_world_tcp",
            }
            (run_dir / "grasp_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=True))
            return str(run_dir)
    ...
    def plan_visual_grasp_tcp_pose(
        ...
        **selection_kwargs,
        ...
        if not selection.get("ok"):
            artifact_dir = self._maybe_save_visual_grasp_artifacts(
                object_name=object_name,
                rgb=rgb,
                mask=mask,
                detection=detection,
            ...
            return {
                ...
                "visual_artifact_dir": artifact_dir,
    ...
    def _plan_from_visual_grasp_selection(
        self,
        ...
        *,
        ...
        insert_waypoints: int,
    ) -> dict[str, Any]:
        """Reuse one visual/GraspNet sample with a different ranked TCP candidate."""
        grasp_tcp_pose = selection["grasp_tcp_pose"]
        grasp_pos, grasp_quat = self._coerce_pose(grasp_tcp_pose)
        tcp_offset = self.get_tcp_offset_eef(arm=arm)
        tcp_axis_world = x2_vision.quat_xyzw_to_matrix(grasp_quat) @ tcp_offset
        tcp_axis_world = tcp_axis_world / max(float(np.linalg.norm(tcp_axis_world)), 1e-12)
        precontact_tcp_pose = (
            grasp_pos - tcp_axis_world * float(precontact_distance),
            grasp_quat.copy(),
        )
        insertion_waypoints = []
        for idx, distance in enumerate(np.linspace(float(precontact_distance), 0.0, int(insert_waypoints) + 1)[1:]):
            insertion_waypoints.append(
                {
                    ...
                    "distance_m": float(distance),
                    "tcp_pose": (
                        grasp_pos - tcp_axis_world * float(distance),
                        grasp_quat.copy(),
                    ),
                }
            )
        ...
        plan.update(
            {
                ...
                "insertion_waypoints": insertion_waypoints,
                "tcp_axis_world": tcp_axis_world,
                "candidate_reuse": "same_visual_graspnet_sample",
            }
        )
        return plan
    ...
    def execute_tcp_grasp_plan(
        ...
        *,
        ...
        place_transfer_waypoints: int = 5,
        settle_steps: int = 16,
        ...
        if before_close_error["tcp_error_m"] > fine_align_target_tcp_threshold:
            fine_align_ok = self.move_tcp_joint_ik(
                ...
                max_joint_step=max(0.004, float(insert_max_joint_step) * 0.6),
                max_steps=560,
                settle_steps=max(20, int(settle_steps)),
                hold_steps_per_waypoint=max(8, int(insert_hold_steps_per_waypoint)),
            )
            before_close_tcp_pose = self.get_current_tcp_pose(arm=arm)
            before_close_error = {
                "tcp_error_m": float(np.linalg.norm(before_close_tcp_pose[0] - grasp_pos)),
        ...
        if skip_place_due_to_empty_gripper:
            ...
            transfer_waypoint_results = []
            transfer_start_tcp_pose = self.get_current_tcp_pose(arm=arm)
            transfer_start_pos = np.asarray(transfer_start_tcp_pose[0], dtype=np.float64).reshape(3)
            transfer_start_quat = np.asarray(transfer_start_tcp_pose[1], dtype=np.float64).reshape(4)
            place_pre_pos = np.asarray(place_pre_tcp_pose[0], dtype=np.float64).reshape(3)
            safe_transfer_z = max(float(transfer_start_pos[2]), float(place_pre_pos[2]))
            safe_start_tcp_pose = (
                np.array([transfer_start_pos[0], transfer_start_pos[1], safe_transfer_z], dtype=np.float64),
                transfer_start_quat.copy(),
            )
            safe_start_ok = self.move_tcp_joint_ik(
                safe_start_tcp_pose,
                arm=arm,
                pos_thresh=0.02,
                ori_thresh=0.35,
            ...
            transfer_waypoint_results.append(
                {
                    "name": "transfer_safe_start",
                    "ok": bool(safe_start_ok),
                    "target_tcp_pose": safe_start_tcp_pose,
                    "reached_tcp_pose": self.get_current_tcp_pose(arm=arm),
                }
            )
            safe_goal_pos = np.array([place_pre_pos[0], place_pre_pos[1], safe_transfer_z], dtype=np.float64)
            current_transfer_pos = np.asarray(self.get_current_tcp_pose(arm=arm)[0], dtype=np.float64).reshape(3)
            lateral_ok = True
            num_transfer_waypoints = max(1, int(place_transfer_waypoints))
            for waypoint_idx, alpha in enumerate(np.linspace(0.0, 1.0, num_transfer_waypoints + 1)[1:]):
                waypoint_pos = (1.0 - float(alpha)) * current_transfer_pos + float(alpha) * safe_goal_pos
                waypoint_pose = (waypoint_pos, release_quat.copy())
                waypoint_ok = self.move_tcp_joint_ik(
                    waypoint_pose,
                    arm=arm,
                    pos_thresh=0.02,
                    ori_thresh=0.35,
                ...
                lateral_ok = bool(lateral_ok and waypoint_ok)
                transfer_waypoint_results.append(
                    {
                        "name": f"transfer_lateral_{waypoint_idx:02d}",
                        "ok": bool(waypoint_ok),
                        "target_tcp_pose": waypoint_pose,
                        "reached_tcp_pose": self.get_current_tcp_pose(arm=arm),
                    }
                )
            place_pre_ok = self.move_tcp_joint_ik(
                place_pre_tcp_pose,
                arm=arm,
                pos_thresh=0.02,
                ori_thresh=0.35,
            ...
            transfer_waypoint_results.append(
                {
                    ...
                    "ok": bool(place_pre_ok),
                    "target_tcp_pose": place_pre_tcp_pose,
                    "reached_tcp_pose": self.get_current_tcp_pose(arm=arm),
                }
            )
            place_pre_ok = bool(safe_start_ok and lateral_ok and place_pre_ok)
            object_corrections = []
        ...
        *,
        ...
        for attempt_idx, candidate_index in enumerate(indices):
            if base_plan is None:
                ...
                pose_estimate = base_plan.get("pose_estimate") or {}
                selection = self.select_adapted_graspnet_tcp_candidate(
                    base_plan.get("graspnet") or {},
                    np.asarray(pose_estimate.get("position_world"), dtype=np.float64).reshape(3),
                    arm=arm,
                    index=int(candidate_index),
                    reference_quat_xyzw=reference_quat,
                    proxy_guard_object_size=0.04,
                    proxy_guard_approach_distance=0.08,
                    proxy_guard_first_insert_fraction=0.9,
                )
                if not selection.get("ok"):
                    plan = {
                        "ok": False,
                    ...
                    plan = self._plan_from_visual_grasp_selection(
                        base_plan,
                        selection,
                        arm=arm,
                        precontact_distance=0.08,
                ...
                attempts.append(
                    {
                        ...
                        "candidate_reuse": "same_visual_graspnet_sample" if base_plan is not None else "initial_visual_graspnet_sample",
                    }
                )
                continue
            ...
            execution = self.execute_tcp_grasp_plan(
                ...
                insert_hold_steps_per_waypoint=8,
                close_hold_steps=30,
                release_hold_steps=24,
                final_tcp_threshold=0.023,
            ...
            attempts.append(
                {
                    ...
                    "candidate_reuse": plan.get("candidate_reuse", "initial_visual_graspnet_sample"),
                    "selected_rank": (plan.get("selection") or {}).get("selected_rank"),
                    "selected_raw_index": (plan.get("selection") or {}).get("selected_raw_index"),
...
class X2PickPlaceApi(X2ControlApi):
    ...
    This wrapper intentionally exposes only task-level pick-place primitives
    and the reusable visual pick-place primitive. The full ``X2ControlApi``
    remains available for debugging and lower-level experiments, but ordinary
    CaP-X task code should prefer this reduced API so generated programs do
    not need to reason about cameras, GraspNet frames, TCP-to-EEF conversion,
    IK, or trajectory optimization.
    """

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._x2_pick_place_red_cube_result: dict[str, Any] | None = None
        self._x2_pick_place_red_cube_target: np.ndarray | None = None

    def functions(self) -> dict[str, Any]:
        """Return the reduced X2 primitive set exposed to generated task code.

        The full ``X2ControlApi`` still exposes camera, visual-chain, IK, and
        gripper debugging helpers.  This reduced task API keeps ordinary LLM
        programs at the CaP-X primitive level: choose a task primitive or the
        reusable visual pick-place primitive, then inspect the returned dict.
        """
        return {
            "pick_and_place_red_cube": self.pick_and_place_red_cube,
            "pick_and_place_red_cube_to_left_target": self.pick_and_place_red_cube_to_left_target,
            "pick_and_place_red_cube_to_right_target": self.pick_and_place_red_cube_to_right_target,
            "pick_and_place_visual_object": self.pick_and_place_visual_object,
        }
    ...
    def pick_and_place_red_cube(
        ...
        *,
        ...
        if (
            self._x2_pick_place_red_cube_result is not None
            and self._x2_pick_place_red_cube_target is not None
            and not bool(force)
            and np.allclose(target, self._x2_pick_place_red_cube_target, atol=1e-6)
        ):
        ...
        self._x2_pick_place_red_cube_target = target.copy()
        return result

    def pick_and_place_red_cube_to_left_target(
        self,
        *,
        place_position_threshold: float = 0.10,
        force: bool = False,
    ) -> dict[str, Any]:
        """Pick the red cube and place it at the left tabletop target marker."""
        result = self.pick_and_place_red_cube(
            place_position=np.array([0.27, 0.055, 0.921], dtype=np.float64),
            place_position_threshold=place_position_threshold,
            force=force,
        )
        result = dict(result)
        result["task_primitive"] = "pick_and_place_red_cube_to_left_target"
        result["target_name"] = "left"
        return result

    def pick_and_place_red_cube_to_right_target(
        self,
        *,
        place_position_threshold: float = 0.10,
        force: bool = False,
    ) -> dict[str, Any]:
        """Pick the red cube and place it at the right tabletop target marker."""
        result = self.pick_and_place_red_cube(
            place_position=np.array([0.37, 0.055, 0.921], dtype=np.float64),
            place_position_threshold=place_position_threshold,
            force=force,
        )
        result = dict(result)
        result["task_primitive"] = "pick_and_place_red_cube_to_right_target"
        result["target_name"] = "right"
        return result
