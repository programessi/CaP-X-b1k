...
def _target_position(target_name: str) -> np.ndarray:
    if target_name == "left":
        return LEFT_PLACE_POSITION
    if target_name == "right":
        return RIGHT_PLACE_POSITION
    raise ValueError(f"Unsupported target name: {target_name!r}")


def _target_marker_name(target_name: str) -> str:
    if target_name == "left":
        return LEFT_MARKER_NAME
    if target_name == "right":
        return RIGHT_MARKER_NAME
    raise ValueError(f"Unsupported target name: {target_name!r}")


def _primitive_name(target_name: str) -> str:
    if target_name == "left":
        return "pick_and_place_red_cube_to_left_target"
    if target_name == "right":
        return "pick_and_place_red_cube_to_right_target"
    raise ValueError(f"Unsupported target name: {target_name!r}")


def _make_prompt(target_name: str) -> str:
    primitive = _primitive_name(target_name)
    marker_name = _target_marker_name(target_name)
    target_label = target_name.upper()
    return f"""
You are controlling a fixed-base X2 robot in a simple tabletop pick-place task.
Goal: pick up the movable red cube named "{OBJECT_NAME}" and place it at the {target_label} target marker named "{marker_name}".
...
- {primitive}() internally plans X2 executable TCP poses in world frame, T_world_tcp.
...
Write exactly one call to {primitive}(); do not call it more than once.
Recommended code:
RESULT = {primitive}()
...
for attempt in attempts:
    attempt_error = attempt.get("before_close_error") or {{}}
    print("X2_TWO_TARGET_ATTEMPT "
...
def _make_oracle_code(target_name: str) -> str:
    primitive = _primitive_name(target_name)
    return f"""
RESULT = {primitive}()
execution = RESULT.get("execution", {{}})
...
class X2PickPlaceRedCubeTwoTargetsBaseCodeEnv(CodeExecutionEnvBase):
    """Shared X2 two-target red-cube task implementation."""
    ...
    def place_error(self) -> float | None:
        ...
        if pos is None:
            return None
        return float(np.linalg.norm(pos - _target_position(self.target_name)))
    ...
    def _last_execution_success(self) -> bool:
        ...
        if result.get("ok") is not True:
            return False
        if result.get("target_name") != self.target_name:
            return False
    ...
    def step(self, action: str):
        ...
        err = self.place_error()
        result, execution = self._last_execution_result()
        info[f"{self.target_name}_place_error_m"] = err
...
class X2PickPlaceRedCubeTwoTargetsCodeEnv(X2PickPlaceRedCubeTwoTargetsBaseCodeEnv):
    """X2 CaP-X task for placing the red cube at the right target."""

    target_name = "right"
    prompt = _make_prompt(target_name)
    oracle_code = _make_oracle_code(target_name)


class X2PickPlaceRedCubeTwoTargetsLeftCodeEnv(X2PickPlaceRedCubeTwoTargetsBaseCodeEnv):
    """X2 CaP-X task for placing the red cube at the left target."""
