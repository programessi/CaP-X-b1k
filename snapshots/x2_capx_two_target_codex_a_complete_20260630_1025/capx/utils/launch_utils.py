...
def _extract_code(content: str) -> list[str]:
    ...
    content = _remove_reasoning_blocks(content)
    fragments: list[str] = []
...
def _remove_reasoning_blocks(text: str) -> str:
    """Remove model reasoning blocks before extracting executable code."""
    import re

    if not isinstance(text, str):
        text = str(text)
    text = re.sub(r"<think\b[^>]*>.*?</think>", "", text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r"igid.*?igid", "", text, flags=re.DOTALL | re.IGNORECASE)
    match = re.search(r"<think\b[^>]*>|igid", text, flags=re.IGNORECASE)
    if match:
