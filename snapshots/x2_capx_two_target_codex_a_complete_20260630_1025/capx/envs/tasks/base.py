...
class CodeExecutionEnvBase(Env):
    ...
    def __init__(self, cfg: CodeExecEnvConfig) -> None:
        ...
        self._system_prompt = (
            ...
            "Do not output explanations, markdown fences, natural language, or think tags. "
    ...
    def _clean_model_code(self, code: str) -> str:
        ...
        Reasoning-style models such as MiniMax-M2.7 may return
        natural-language reasoning, <think>...</think> blocks, markdown fences,
        or a mixture of prose and code. This method keeps only executable
        Python code as much as possible.

        The function is intentionally conservative:
        - If a fenced Python code block exists, use it.
        - Remove complete and unclosed <think> blocks.
        ...
        # Prefer fenced Python code blocks from the original response. This
        # handles responses that include reasoning text followed by a fenced
        # Python block.
        ...
        # Remove complete reasoning blocks. Keep the legacy "igid" cleanup too:
        # previous prompts used that token as a workaround for reasoning tags.
        ...
        # Remove unclosed reasoning blocks. If anything remains before the block,
        # keep that; otherwise discard the text rather than executing prose.
        think_match = re.search(r"<think\b[^>]*>|igid", code, flags=re.IGNORECASE)
        if think_match:
            code = code[:think_match.start()].strip()
        code = re.sub(r"</think>", "", code, flags=re.IGNORECASE).strip()
