...
def solve_joint_trajopt(
    ...
    """Optimize a collision-aware joint trajectory between fixed endpoints."""
    del dt
    if isinstance(start_cfg, onp.ndarray):
        np = onp
    elif isinstance(start_cfg, jnp.ndarray):
        np = jnp
    else:
        raise ValueError(f"Invalid type for `ArrayLike`: {type(start_cfg)}")

    start_cfg = jnp.asarray(start_cfg)
    end_cfg = jnp.asarray(end_cfg)
    ...
    def compute_world_coll_residual(
        ...
        coll = robot_coll.get_swept_capsules(robot, vals[prev_traj_vars], vals[curr_traj_vars])
        dist = pk.collision.collide(coll.reshape((-1, 1)), world_coll_obj.reshape((1, -1)))
        colldist = pk.collision.colldist_from_sdf(dist, 0.08)
        return (colldist * 20.0).flatten()
    ...
    factors.extend(
        [
            jaxls.Cost(
                lambda vals, var: ((vals[var] - start_cfg) * 500.0).flatten(),
                (robot.joint_var_cls(0),),
                name="start_joint_constraint",
            ),
            jaxls.Cost(
                lambda vals, var: ((vals[var] - end_cfg) * 500.0).flatten(),
                (robot.joint_var_cls(timesteps - 1),),
                name="end_joint_constraint",
            ),
        ]
    )
    ...
    solution = (
        ...
        .solve(
            initial_vals=jaxls.VarValues.make((traj_vars.with_value(init_traj),)),
            termination=jaxls.TerminationConfig(max_iterations=80),
        )
    )
    return np.array(solution[traj_vars])
