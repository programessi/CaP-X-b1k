"""Diagnose X2 joint-space IK failures by separating solve and execution errors."""
...
POSE_SPECS = [
    ...
    {
        "name": "failed_small_up_safe_x_minus15",
        "delta": [0.0, 0.0, 0.018],
        "quat_xyzw": [0.62914, 0.071791, 0.749614, 0.192634],
    },
    {
        "name": "failed_small_forward_left_mixed",
        "delta": [0.015, -0.01, 0.012],
        "quat_xyzw": [0.607741, 0.262438, 0.748714, 0.034704],
    },
    {
        "name": "small_back_right_mid",
        "delta": [-0.015, 0.012, 0.01],
        "quat_xyzw": [0.623938, 0.172114, 0.755461, 0.101771],
    },
    {
        "name": "small_right_low_roll",
        "delta": [0.008, 0.012, -0.006],
        "quat_xyzw": [0.621289, 0.252274, 0.713342, 0.20372],
    },
    {
        "name": "small_transport_up_yaw",
        "delta": [0.018, 0.008, 0.02],
...
record(
    "multi_pose_contract",
    target_source="current_reset_tcp_anchor_plus_v3_failure_and_success_offsets",
    target_frame="world",
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "ok": False,
        "target_meaning": "diagnostic TCP/finger-center poses in world frame, defined as local offsets from the reset-time TCP anchor; includes v3 failed and passed orientation targets",
    ...
    try:
        ...
        print("[x2-joint-ik-failure-diagnostics] executing injected policy", flush=True)
    ...
    print(f"[x2-joint-ik-failure-diagnostics] wrote {summary_path}", flush=True)
