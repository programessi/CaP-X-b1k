...
if [[ -z "${CODEX_BIN:-}" ]]; then
  if command -v codex-a >/dev/null 2>&1; then
    CODEX_BIN="codex-a"
  else
    CODEX_BIN="codex"
  fi
...
if ! curl -sS --max-time 2 "${HEALTH_URL}" >/dev/null; then
