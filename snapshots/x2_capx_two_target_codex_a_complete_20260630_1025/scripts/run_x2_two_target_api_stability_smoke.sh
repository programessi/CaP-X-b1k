...
run_one() {
  ...
  local config_path="$2"
  local idx="$3"
  local run_name="two_targets_${side}_api_stability_${STAMP}_run$(printf '%02d' "${idx}")"

  echo "[x2-api-stability] side=${side} run=${idx}/${REPEATS}"
  CONFIG_PATH="${config_path}" \
  OUTPUT_DIR="./outputs/stability/${run_name}" \
  VISUAL_ARTIFACT_DIR="${REPO_ROOT}/outputs/x2_visual_artifacts/stability/${run_name}" \
  "${SCRIPT_DIR}/run_x2_two_target_api_non_oracle_smoke.sh"
}
...
echo "[x2-api-stability] done stamp=${STAMP}"
