...
**MiniMax 直连模式：** 如果使用 MiniMax 模型，可用 MiniMax 原生密钥直连（跳过 OpenRouter）：

```bash
# 先创建 .minimaxkey 文件（MiniMax 原生 API key）
echo "sk-api-your-key-here" > .minimaxkey
...
| **X2 tabletop** | 1 个基线任务 | `x2_pick_place_red_cube.yaml` |
